...
app.get('/api/products', async (req, res) => {
 try {
   ...
   const response = await axios.get('https://api.mockaroo.com/api/generate.json?key=ab260320&schema=product&count=20');
...
app.get('/api/messages', async(req, res) => {
  try {
     const response = await axios.get('https://api.mockaroo.com/api/generate.json?key=ab260320&schema=messages&count=10');
     const mockData = response.data;
     res.json(mockData);
  }
